...
const BASE_URL = process.argv[2] || "http://localhost:3000/api/parse";
...
function matchVerdict(actual, expected) {
  ...
  if (expected.verdict && !act.includes(expected.verdict.toUpperCase())) return false;
  return true;
}
...
async function run() {
  ...
  for (const t of tests) {
    ...
    if (ok) {
      pass++;
      console.log(`${green("PASS")} ${t.id}  → verdict=${out.verdict}  ${gray(out.size_hint || "")}`);
    } else {
      console.log(`${red("FAIL")} ${t.id}
  expected: verdict=${t.expected.verdict}${t.expected.size_like ? ` (size~${t.expected.size_like})` : ""}
  got     : verdict=${out.verdict}  size=${out.size_hint}
  gto: ${gray((out.gto_strategy || "").slice(0, 140).replace(/\n/g, " "))}…`);
    }
  }
