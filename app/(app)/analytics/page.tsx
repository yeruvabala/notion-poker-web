    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
          <div className="mb-3 text-sm font-medium text-gray-700">
            Leak Impact (bb lost avg)
          </div>
          <div className="space-y-2">
            {leaks.map((l) => (
              <LeakRowBar key={l.learning_tag} tag={l.learning_tag} bb={l.bb} />
            ))}
            {!leaks.length && <div className="text-sm text-gray-500">No leaks detected for this filter.</div>}
          </div>
        </div>
      </div>
  ...
  return (
    <div className="flex items-center gap-3 rounded-full border border-gray-200 bg-white px-4 py-2 shadow-sm">
      ...
      <div className="text-sm">
        ...
        <div className="text-xs text-gray-500">{fmt(bb)} bb • {n} hands</div>
      </div>
    </div>
  );
}
...
function LeakRowBar({ tag, bb }: { tag: string; bb: number }) {
  const isLoss = bb < 0;
  const width = Math.min(Math.abs(bb) * 8, 100); // visual only
    ...
    <div className="flex items-center gap-3">
      ...
      <div className="flex-1">
        <div
          ...
          style={{ width: `${width}%`, background: color, opacity: 0.25 }}
        />
      </div>
