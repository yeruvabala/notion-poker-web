...
export class RangeEngine {
    ...
    static initializeRange(
        ...
        actionType: 'open' | 'call_open' | '3bet' | 'call_3bet' | 'limp'
    ): Range {
        ...
        for (const hand of allHands) {
            ...
            if (actionType === 'call_open') {
                // Example: Determine if hand is in calling range
                // For now, we use a heuristic or we need to expose more from gtoRanges
                // Let's use getOpeningFrequency as a proxy for raw strength for now
                // REAL IMPLEMENTATION: We need to match the specific scenario
                const openFreq = getOpeningFrequency(hand, villainPosition);
                // Calling range is usually TOP 10-40% excluding TOP 3% (3bets)
                // This is a placeholder logic
                if (openFreq > 0.1 && openFreq < 0.9) weight = 0.8;
                else if (openFreq >= 0.9) weight = 0.2; // Traps
                else weight = 0;
